...
void gpu_buddy_print(struct gpu_buddy *mm)
{
	...
	pr_info("chunk_size: %luKiB, total: %luMiB, free: %luMiB, clear_free: %luMiB\n",
	...
	for (order = mm->max_order; order >= 0; order--) {
		...
		if (free < SZ_1M)
			pr_info("order-%2d free: %8lu KiB, blocks: %lu\n",
				order, free >> 10, count);
		else
			pr_info("order-%2d free: %8lu MiB, blocks: %lu\n",
				order, free >> 20, count);
	}
}
